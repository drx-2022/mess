...
import { archive, chat, database, group, inbox, moon, sun, users } from "@/utils/Icons"
...
function Sidebar() {
  ...
  const { allChatsData, handleSelectedChat, selectedChat, showCreateGroup, setShowCreateGroup } = useChatContext()
  const { showProfile, handleProfileToggle, handleFriendProfile, handleViewChange, currentView } = useGlobalContext()
    ...
    <div className="w-[25rem] flex border-r-2 border-white dark:border-[#3C3C3C]/60">
      <div className="p-4 flex flex-col justify-between items-center border-r-2 border-white dark:border-[#3C3C3C]/60">
        ...
        <div className="p-2 text-[#454e56] text-xl flex flex-col gap-2 border-2 border-white dark:border-[#3C3C3C]/65 rounded-[30px] shadow-sm dark:text-white/65">
          ...
          <button className={`${user?.theme === "dark" ? "text-white" : ""}`} onClick={() => darkTheme()}>
            {moon}
          </button>
        </div>
      </div>
      <div className="pb-4 flex-1">
        <div className="flex justify-between items-center px-4 mt-6">
          <h2 className={`font-bold text-2xl ${gradientText} dark:text-white`}>Messages</h2>
          <button
            onClick={() => setShowCreateGroup(true)}
            className="p-2 bg-[#7263f3] text-white rounded-full hover:bg-[#6253e3] transition-colors"
          >
            {users}
          </button>
        </div>
