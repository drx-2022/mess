...
function ChatItem({ user, active, onClick, chatId }: ChatItemProps) {
  ...
  */
  ...
  const isUserOnline = !isGroup && onlineUsers?.find((onlineUser: IUser) => onlineUser._id === user._id)
    ...
    >
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex justify-between items-center">
          ...
          <p className="text-[#aaa] text-sm">{formatDateBasedOnTime(lastMessage?.createdAt)}</p>
        </div>
        <div className="flex justify-between items-center">
